  ...
  try {
    ...
    if (payment.status === 'approved' && payment.external_reference) {
      const patchRes = await fetch(
        SUPA_URL + '/rest/v1/cobrancas?id=eq.' + payment.external_reference +
        '&status=in.(a_receber,atrasado)',
        {
          method: 'PATCH',
          headers: {
      ...
      console.log('Webhook MP: payment ' + payment_id +
        ' | atualizadas: ' + (Array.isArray(updated) ? updated.length : 0) +
        ' | HTTP ' + patchRes.status);
    }
    return { statusCode: 200, body: 'OK' };
  } catch (err) {
    console.log('Webhook MP erro: ' + (err && err.message));
    return { statusCode: 200, body: 'OK' };
  }
};
